...
The CLI is included in the HamBros monorepo as `@hambros/cli`, and `install.sh` installs it onto your `PATH` as `hambros`:
...
When a production client build exists, `hambros start` runs the source server with `NODE_ENV=production` so the built UI is served correctly. If compiled server artifacts are the only available entrypoint, it falls back to the built server.
