...
describe('createGeminiAcpSession', () => {
  it('tears down and rethrows a friendly startup error when ensureConnected rejects', async () => {
    const runtime = createRuntime()
    runtime.ensureConnected.mockRejectedValue(new Error('spawn gemini ENOENT'))
    const runtimeFactory = vi.fn(() => runtime)

    await expect(createGeminiAcpSession(
      'gemini-worker-1',
      'default',
      '',
      '/tmp/gemini-worker',
    ...
    )).rejects.toThrow('Gemini runtime failed to start: spawn gemini ENOENT')
